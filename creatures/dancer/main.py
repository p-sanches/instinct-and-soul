...
class Mem:
    """Bounded named-slot memory that survives instinct hot-swap.

    Lives at main.py module scope, so the soul's run() can be cancelled
    and re-exec'd without losing what was pushed.
    """
    DEFAULT_MAXLEN = 300
    MAX_MAXLEN = 1000
    MAX_SLOTS = 8
    ...
    @classmethod
    def push(cls, slot, value, maxlen=None):
        if slot not in cls._slots:
            if len(cls._slots) >= cls.MAX_SLOTS:
                return
            cls._slots[slot] = []
            cls._caps[slot] = min(maxlen or cls.DEFAULT_MAXLEN, cls.MAX_MAXLEN)
        elif maxlen is not None:
            new_cap = min(maxlen, cls.MAX_MAXLEN)
            cls._caps[slot] = new_cap
            buf = cls._slots[slot]
            while len(buf) > new_cap:
                buf.pop(0)
        buf = cls._slots[slot]
        buf.append(value)
        if len(buf) > cls._caps[slot]:
            buf.pop(0)

    @classmethod
    def recent(cls, slot, n=None):
        buf = cls._slots.get(slot)
        if not buf:
            return []
    ...
    @classmethod
    def clear(cls, slot=None):
        if slot is None:
            cls._slots = {}
            cls._caps = {}
        elif slot in cls._slots:
            cls._slots[slot] = []

    @classmethod
    def snapshot(cls):
        return {k: list(v) for k, v in cls._slots.items()}
...
async def mem_snapshot_loop():
    while True:
        ...
        try:
            if ws and Mem._slots:
                ws.send("MEM:" + json.dumps(Mem.snapshot()))
        except Exception as e:
